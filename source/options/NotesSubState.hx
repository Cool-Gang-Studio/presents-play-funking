		...
		if(holdingOnObj != null)
		{
			...
			else if (generalMoved || generalPressed)
			{
				...
				else if (holdingOnObj == colorWheel)
				{
					...
					var hue:Float = FlxMath.wrap(FlxMath.wrap(Std.int(FlxPointFunctions.degreesTo(mouse, center)), 0, 360) - 90, 0, 360);
					var sat:Float = FlxMath.bound(FlxPointFunctions.dist(mouse, center) / colorWheel.width*2, 0, 1);
					//trace('$hue, $sat');
...
// HAVE TO PUT THIS STUFF TEMPORARILY HERE SO I CAN RECREATE FUNCTIONS AVAILIBLE IN FLIXEL 5
class FlxPointFunctions
{
	public static inline function degreesTo(point1:FlxPoint, point2:FlxPoint):Float
	{
		return degreesFromOrigin(point2.x - point1.x, point2.y - point1.y);
	}

	public static inline function degreesFromOrigin(x:Float, y:Float):Float
	{
		return angleFromOrigin(x, y, true);
	}

	public static inline function angleFromOrigin(x:Float, y:Float, asDegrees:Bool = false):Float
	{
		return Math.atan2(y, x) * (asDegrees ? FlxAngle.TO_DEG : 1);
	}

	public static inline function dist(p1:FlxPoint, p2:FlxPoint):Float
	{
		return Math.sqrt(distSquared(p1, p2));
	}

	public static inline function distSquared(p1:FlxPoint, p2:FlxPoint):Float
	{
